#!/usr/bin/env python3
"""Freehand v2 Control Events AppSDK lifecycle adapter."""
    ...
    *,
    ...
    record = {
        ...
        "scope": {
            ...
            "feature_id": "v2-control-events",
        },
        "producer": {"adapter": "freehand-v2-control-events-adapter", "identity": producer_identity},
        "result": "pass",
    ...
    if phase == "development_whitebox":
        ...
        record["scope"]["entrypoint"] = "cargo test --manifest-path playground/experiments/v2/control-events/Cargo.toml --test v2_control_event_boundary"
        if artifact_hash:
...
def main() -> None:
    ...
    parser.add_argument("--module", default="v2-control-events")
    parser.add_argument("--attempt", default=None)
    args = parser.parse_args()

    project = pathlib.Path(args.project).resolve()
    if args.module != "v2-control-events":
    ...
    issue_id = "freehand-v2-control-events-milestone"
    experiment_id = f"v2-control-events-{attempt}"
    records_dir = project / ".appsdk" / "records"
    evidence_dir = records_dir / "evidence" / "v2-control-events"
    staging_dir = project / ".appsdk-control" / "transactions" / "v2-control-events" / attempt
    ...
    pre_review_path = records_dir / "pre-review-validation-record-v2-control-events.json"
    ...
    manifest_path = project / "playground" / "experiments" / "v2" / "control-events" / "Cargo.toml"
    test_cmd = [
        "cargo",
        "test",
        "--manifest-path",
    ...
    deploy_dir = pathlib.Path("/tmp") / f"freehand-v2-control-events-{attempt}"
    deploy_dir.mkdir(parents=True, exist_ok=True)
    entrypoint = deploy_dir / "v2-control-events-public"
    artifact = project / "generated" / "modules" / "v2-control-events" / "lib" / "v2-control-events.module"
    if not artifact.exists():
        fail(attempt, "deployment_install", f"missing compiled artifact {artifact}")
    shutil.copy2(artifact, entrypoint)

    environment_id = f"local-freehand-v2-control-events-{attempt}"
    install_input = json.dumps(
        {"action": "emit", "event_id": f"e-install-{attempt}", "correlation_id": f"corr-install-{attempt}", "kind": "PluginInvoked", "owner": "plugin-a"},
        sort_keys=True,
    )
    restart_input = "\n".join([
        json.dumps({"action": "emit", "event_id": f"e-restart-{attempt}", "correlation_id": f"corr-restart-{attempt}", "kind": "PluginInvoked", "owner": "plugin-a"}, sort_keys=True),
        json.dumps({"action": "ack", "event_id": f"e-restart-{attempt}"}, sort_keys=True),
    ])
    blackbox_input = "\n".join([
        json.dumps({"action": "emit", "event_id": f"e-blackbox-{attempt}", "correlation_id": f"corr-blackbox-{attempt}", "kind": "PluginInvoked", "owner": "plugin-a"}, sort_keys=True),
        json.dumps({"action": "ack", "event_id": f"e-blackbox-{attempt}"}, sort_keys=True),
        json.dumps({"action": "complete", "correlation_id": f"corr-blackbox-{attempt}"}, sort_keys=True),
    ])

    install_id = f"ev-v2-control-events-install-{attempt}"
    ...
    restart_id = f"ev-v2-control-events-restart-{attempt}"
    ...
    blackbox_id = f"ev-v2-control-events-blackbox-{attempt}"
    ...
    fix_candidate_id = f"fix-v2-control-events-{attempt}"
    fix_candidate = {
        "$schema": "https://appsdk.local/contracts/records/fix-candidate-record.schema.json",
        "fix_candidate_id": fix_candidate_id,
        "issue_id": issue_id,
        "module_id": "v2-control-events",
        "worktree_id": "playground/v2-control-events-20260903",
        "base_commit": base_commit,
        "head_commit": head_commit,
        "tree_hash": tree_hash,
        "diff_hash": diff_hash,
        "design_id": "docs/design/v2-control-events-owner-binding.md",
    ...
    validation_id = f"prv-v2-control-events-{attempt}"
    pre_review = {
        ...
        "artifact_hash": artifact_hash,
        "whitebox_producer": {"adapter": "freehand-v2-control-events-adapter", "identity": attempt},
        ...
        "deployment": {
            ...
            "entrypoint": str(entrypoint),
            "producer": {"adapter": "freehand-v2-control-events-adapter", "identity": attempt},
    ...
    finalize(records_dir / "fix-candidate-record-v2-control-events.json", fix_candidate)
