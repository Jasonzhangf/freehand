...
- follow-up correction: first implementation must be a fixed hook skeleton. Schema variants and action processors can evolve, but request hooks must inject status schema instructions and action tool specs at fixed positions, and response hooks must parse status, admit action tools, classify errors, decide rhythm, apply owner actions, and project public output in a fixed order. No schema/action implementation may bypass these hooks.
