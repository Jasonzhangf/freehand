...
- OpenMinis lifecycle evidence uses one verifier-report admission path for repository, WebUI, Android-device, and legacy no-touch gates. Do not reject a required gate by id: require its code-locked command/proof/verifier identity, gate-specific assertions, successful process result, distinct report digest, exact source commit/tree, and clean source-attestation boundary; otherwise terminal lifecycle states become unrepresentable.
