...
- For an S-profile Agent with `relay_url` and `relay_token_env`, provision both
  the Relay account token and the local `FREEHAND_ADP_AUTH_TOKEN` before service
  restart. Keep raw credentials out of repo/config truth; a mode-0600 launchd
  env may resolve named macOS Keychain entries at source time. Restart Master
  with `restartS` and each Worker with its exact
  `FREEHAND_WORKER_AGENT=<id> restartWorkerS` service command. Product proof
  requires all configured roles online, HTTP and ADP proxy success, one exact
  Worker offline/404 transition, service-scoped recovery, and installed-binary
  SHA equality.
...
- Relay work status/count must cross the daemon boundary as a dedicated typed control projection, never a tuple inferred from UI/ADP payloads. Bind turn-to-activity, AgentLifecycle-to-activity, and activity-to-presence as separate resource operations with allowed direct relations, source-edge rows, function/mainline/test backlinks, and an executable verification-map gate. Presence source failure closes the Relay client; never reuse the static bootstrap heartbeat as fallback truth.
- A newly authenticated Relay control socket for the same account and Agent is a new generation, not a permanent duplicate rejection. The registry owner must atomically replace stale control truth, detach old data/error attachments, and fail every old pending exchange through typed delivery. Cleanup and heartbeat from the old control generation must be fenced so they cannot remove or overwrite the new generation. Prove this without restarting Relay: connect a second client with the same identity, observe it become current, and verify directory and proxy truth remain live.

## Generation-bound channel admission

For reconnectable Relay lifecycles, treat the server-issued control generation as the owner token for dependent data/error channels. Pass it over a dedicated handshake header, validate it inside the single tunnel-registry mutation, and test both current-generation admission and stale-generation rejection. Never admit a dependent channel by checking only that some control channel exists.
