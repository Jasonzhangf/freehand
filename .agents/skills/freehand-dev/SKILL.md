...
- WebUI Master/Worker path-diagnostic proof is not closed by a Worker blocked transcript alone. Verify the current task id end-to-end: parent session renders `ToolPending` as `waiting lifecycle` / `Lifecycle running`, Worker detail renders `blocked` at assistant/final/bottom status, Master lifecycle makes a real `task(op="append")` blocked decision that produces `TaskProgressed`, internal Worker prompts stay hidden, and global session list has no top-level `worker-task-*`.
