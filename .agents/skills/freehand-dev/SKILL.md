...
- Master/Worker path delegation must be locked on both sides. Master preserves user-supplied external repo paths and dispatches instead of probing outside `~/.freehand`; Worker expands leading `~`, canonicalizes through symlinks, preserves requested `target_cwd`, and reports requested plus canonical path evidence before analysis.
- Do not invent `/workspace`, `/tmp`, or sibling output directories when the user supplied a repo path. Missing target paths must block with exact original/expanded/canonicalization evidence rather than broad searching or path substitution.
- Worker `target_cwd` is the existing workspace/repository root, not a deliverable/output directory. Preflight failures must classify missing parent, missing workspace under an existing parent, permission/boundary denial, not-directory, and generic canonicalization failure; never collapse these into bare "path not found" or use them as evidence that the user's intended repository is unavailable.
