...
| why not violation | Current claims are scoped and verified: one configured Slave daemon runs a production Worker claim/heartbeat/execute/report loop, and a real-provider Master request completed one external-cwd task through Worker review, Master approval/close, artifact verification, and same-id Worker restart recovery. Multiple Worker resources, background Master scheduling, and retry/reassignment orchestration are not claimed. |
| risk | The single-Worker execution loop is real, but Worker process lifecycle is still manually started for the current proof. There is no daemon/service manager for a configured Worker pool, no background Master scheduler outside an active model turn, and no production reject/retry/reassignment or typed takeover proof. |
| gate | Current gates lock owner maps, role-specific tool surfaces, Worker claim/heartbeat/review-ready/blocked behavior, Slave daemon mode, async blocking boundary, real-provider single-Worker artifact proof, and same-id history recovery. No gate currently requires Worker service auto-start/restart, pool allocation/recycling, background Master timeout polling, or production reject/retry/reassignment. |
| current producers | `ProductionWorkerRunner`, `run_worker_mode`, `run_blocking_worker_service`, `TaskRuntime::claim_next_task`, Worker heartbeat renewal, Worker `ReviewReady`/`Blocked` execution facts, `TaskRuntime::query_task_board`, `TaskRuntime::query_agent_board`, `TaskRuntime::query_event_inbox`, `TaskRuntime::run_master_poll`, worker-control ledger/snapshots, CLI Phase 1/2 samples, WebUI Phase 2 projection |
| missing coverage | managed Worker process auto-start/restart and health truth, multiple configured Worker pool acquisition/release, background Master scheduler and timeout polling outside an active user turn, production blocked/reject/retry/reassignment/takeover loops, formal current-source research/document E2E, browser-visible WebUI proof for the production Worker flow, Android true-device Phase 2 projection proof if mobile dashboard changes are claimed |
...
| priority | high — single configured Worker execution is closed; next foundation step is managed resources plus background Master orchestration |
| closure path | 1) add managed Worker service lifecycle and queryable health 2) add configured pool allocation/release 3) add background Master poll/timeout scheduling outside active turns 4) prove blocked/reject/retry/reassignment/takeover with same ids 5) close formal current-source research and browser-visible WebUI evidence |
