...
| gap kind | Phase 1/2A/2B/2C/2D foundation, production Master evaluation, Worker runner, config-compiled multi-Worker routing, and isolated three-process controlled-provider closure are implemented. Remaining gap is managed-service and real-provider production lifecycle closure. |
| why not violation | Current claims remain scoped: `paired_agents` compiles an ordered Worker set, Master task mutation accepts only set members, each Slave consumes exactly one Master, launchd names Worker services per agent, and the isolated online verifier proves three distinct Worker processes/agents/executions with reject/rework/next-round evaluation. Production promotion remains open because launchd-managed health/restart, real-provider recovery, and typed takeover are not closed. |
| risk | The controlled online topology is real process/runtime truth, but it does not prove three launchd-managed services remain healthy through crash/restart or that real providers preserve the same task/execution/agent identities through recovery. |
| gate | Current gates lock owner maps, unique multi-peer config validation, legacy singular schema rejection, Worker-set assignment admission, role-specific tool surfaces, Worker claim/heartbeat/review-ready/blocked behavior, Slave daemon mode, async blocking boundary, parent evaluation/rework semantics, agent-specific launchd naming, and an isolated online three-PID/three-agent/no-cross-claim proof. No gate yet proves managed health/recycling or real-provider crash/takeover. |
| current producers | `LoadedConfig::select_agent`, `ProductionMasterRunner`, `ProductionWorkerRunner`, `run_master_mode`, `run_worker_mode`, `configured_worker_task_boundary_failure`, `TaskRuntime::claim_next_task`, Worker heartbeat renewal, Worker `ReviewReady`/`Interrupted`/`Blocked` execution facts, durable parent evaluation, TaskBoard/AgentBoard/EventInbox truth, agent-specific Worker launchd service files, WebUI current-session projection |
| missing coverage | managed Worker auto-start/restart and health truth, pool allocation/release under concurrent tasks, real-provider online proof for review close/reject retry/crash recovery/blocked handling, typed takeover, formal current-source research/document E2E, browser-visible same-session WebUI proof, multi-BigTask context switching, and cross-machine Worker transport |
...
| closure path | 1) add queryable managed Worker health/restart truth 2) prove three agent-specific launchd services and same-id crash recovery 3) prove real-provider blocked/reject/retry/crash/reassignment/takeover with same ids 4) close formal current-source research and browser-visible same-session evidence 5) upgrade cross-machine Worker transport from the remaining singular node transport model |
