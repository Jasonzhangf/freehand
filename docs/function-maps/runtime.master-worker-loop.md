...
- When a child remains `TaskBlocked` after the Master has persisted an explicit
  ...
  reviewable, the runner starts a user-visible blocked follow-up turn in the
  original parent session. The follow-up carries original user objective truth,
  Worker blocker evidence, and the Master blocked decision so the parent
  session closes observably as blocked instead of staying on stale waiting
  lifecycle text.
