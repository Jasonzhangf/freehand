...
- module registry: `docs/module-registry/runtime.master-worker-loop.json`
- verification map: `docs/verification-maps/runtime.master-worker-loop.json`
...
| 02p | `ProductionWorkerRunner::current_agent_activity` | `crates/freehand-runtime/src/worker_runner.rs` | read Worker AgentLifecycle truth and project Assigned/Blocked/WaitingReview/Recovering as Waiting, active execution as Running, failure as Error, and terminal or unbound states as Idle for Relay presence | Worker identity + read-only TaskRuntime lifecycle snapshot | typed Worker activity projection with Assigned counted as Waiting/1, or explicit lifecycle read error | daemon Relay presence source | `TaskRuntime::query_agent_lifecycle` | bound |
