...
- resource operations:
  ...
  - paged session-list bridge references `reason.persistence` owner operation `session.list_persisted_page`
...
| 34 | `RuntimeCommandDispatcher::query_runtime` -> `visible_session_list_page` / `session_list_page_request_from_ui` / `session_summary_to_ui` | `crates/freehand-runtime/src/lib.rs` -> `crates/freehand-runtime/src/session_paging.rs` | route `QuerySessionListPage` through the runtime-owned paging module to reason-owned summary paging, map validated protocol direction/cursor/limit, compact internal `worker-task-*` / `master-lifecycle-*` / `master-timer-*` rows before the public page boundary, refresh metadata sidecars, and project only bounded top-level summaries with a cursor anchored to the last visible session; unavailable user sessions remain explicit facts and no full transcript restore occurs in the list path | archived space plus validated latest/older page request | `UiSessionListPageProjection` with ordered top-level summaries, visible-session cursor, has-older, and unavailable ids | ADP query transport / WebUI | `ReasonPersistence::list_persisted_sessions_page` + ui.protocol DTO | bound |
