...
| 34 | `RuntimeCommandDispatcher::query_runtime` / `session_list_page_request_from_ui` / `session_summary_to_ui` | `crates/freehand-runtime/src/lib.rs` | route `QuerySessionListPage` to reason-owned summary paging, map validated protocol direction/cursor/limit, refresh metadata sidecars, and project only bounded metadata summaries with owner page facts; unavailable sessions remain explicit facts and no full transcript restore occurs in the list path | archived space plus validated latest/older page request | `UiSessionListPageProjection` with ordered summaries, cursor, has-older, and unavailable ids | ADP query transport / WebUI | `ReasonPersistence::list_persisted_sessions_page` + ui.protocol DTO | bound |
