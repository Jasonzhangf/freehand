...
| 11c | `UiProtocolState::apply_completion_schema_retry_waiting` | `crates/freehand-ui-protocol/src/lib.rs` | project completion-schema rejection retry as user-visible waiting state | schema retry count + issue summary | queryable/subscribable turn projection with repair-feedback waiting detail | runtime bridge | protocol state | bound |
