...
- `UiRuntimeQueryPort::query_runtime`
  - owner: `crates/freehand-ui-protocol/src/lib.rs`
  - purpose: let app transports ask runtime owners for read-only query projections such as task list/history and error-center events before falling back to protocol-state queries
...
| 11c | `UiProtocolState::apply_completion_schema_retry_waiting` | `crates/freehand-ui-protocol/src/lib.rs` | project completion-schema rejection retry as user-visible waiting state | schema retry count + issue summary | queryable/subscribable turn projection with `kind=SchemaRetry` and compact `schema retry #N: issue` detail | runtime bridge | protocol state | bound |
