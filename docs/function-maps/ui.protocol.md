...
- model request lifecycle is projected as `UiModelRequestActivity` inside `UiTurnProjection` when runtime reports the provider request has been built and sent, or when completion-schema validation rejects a model terminal block and runtime sends repair feedback back to the model; it clears when response/tool/usage/terminal/error projection arrives
...
- empty session ids and empty session titles are rejected at the protocol boundary for session management commands
...
| 11c | `UiProtocolState::apply_completion_schema_retry_waiting` | `crates/freehand-ui-protocol/src/lib.rs` | project completion-schema rejection retry as user-visible waiting state | schema retry count + issue summary | queryable/subscribable turn projection with compact `schema retry #N: issue` detail | runtime bridge | protocol state | bound |
