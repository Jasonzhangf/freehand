...
| 09 | `run_adp_turn_sample_async` | `apps/freehand-cli/src/main.rs` | submit success/failure sample prompts over ADP and verify matching terminal projection | ADP WebSocket URL + sample kind | observed success projection or recovered failed-tool-result projection | ADP sample runner | daemon `/adp` | bound |
