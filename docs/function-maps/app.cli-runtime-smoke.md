...
- for master-worker autonomy sample, CLI submits only one user prompt through ADP `SubmitUserInput`; the live model/tool loop must call the single owner-scoped `task(op=...)` tool to create/assign/claim tasks, record execution facts, review, reject, retry, approve, and close against the configured Worker; CLI then verifies transcript tool activity plus TaskBoard, AgentBoard presence, AgentLifecycle queryability, and TaskHistory owner truth. Scenario terminal truth is task-scoped, not inferred from the configured Worker's latest global lifecycle state.
...
- for normal S-profile master-worker E2E proof, `scripts/verify-normal-master-worker-e2e.sh` starts current S-profile master/worker services, runs the SubmitUserInput autonomy fixture, then verifies production background rejected retry, blocked decision, and Worker crash recovery branches through ADP TaskHistory
...
- normal master-worker E2E verifier prints the autonomy output, one compact branch line for rejected retry / blocked decision / crash recovery, ADP TaskHistory for each branch, and a final `normal_master_worker_e2e_ok`
...
- task restart seed commands fail on non-master agent selection, missing pair
  token/config, duplicate task id, unavailable worker, failed claim, failed
  review submission/rejection, or failed blocked execution-fact persistence;
  they must not write task JSON directly
...
- master-worker autonomy sample fails if CLI emits task mutation commands instead of SubmitUserInput in the mock proof, if transcript lacks the submitted prompt, terminal success, or enough `task` tool executions, if TaskBoard task status/assignee/execution mismatches expected scenario truth, if configured Worker presence/lifecycle queryability is missing, if forbidden events appear, if required ordered events are missing, or if same-id restart verification mismatches
- master-worker autonomy online verifier fails if the provider fixture does not receive exactly 24 `/v1/messages` attempts for all scenarios, if any scenario output is missing, if status/tool/history evidence is missing, or if restart verify cannot query the same owner truth
...
| 19c | `run_normal_master_worker_e2e` | `scripts/verify-normal-master-worker-e2e.sh` | execute the normal S-profile master-worker gate from user-input autonomy through production background branch recovery proofs | S-profile master/worker services + ADP URL | terminal-facing normal E2E proof and branch TaskHistory summaries | operator/agent verifier | daemon `/adp`, launchd services, and Task Center truth | bound |
...
- normal S-profile master-worker E2E verifier is implemented for repeatable local acceptance before manual WebUI testing
