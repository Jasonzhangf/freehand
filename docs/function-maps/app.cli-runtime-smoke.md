...
- for strict Master restart recovery proof, CLI `task-restart-seed-review`
  uses `TaskRuntime` directly while the daemon is stopped to seed
  `TaskReviewSubmitted` owner truth; after daemon restart, the Master lifecycle
  runner must consume the persisted review event and close or reject through
  normal Task Center truth
...
- task restart seed review prints task id, worker id, execution id, and seeded
  event types; it does not approve, close, or run model/protocol transport
...
- task restart seed review fails on non-master agent selection, missing pair
  token/config, duplicate task id, unavailable worker, failed claim, or failed
  review submission; it must not write task JSON directly
...
| 09d | `run_task_restart_seed_review` | `apps/freehand-cli/src/main.rs` | seed review-ready Task Center truth while daemon is stopped for strict restart recovery proof | master agent + task/worker/execution/cwd/summary | terminal-facing seeded event order | CLI dispatcher | `freehand-task::TaskRuntime` | bound |
...
- CLI task restart seed review is implemented for deterministic strict Master
  restart recovery proof; it writes through `TaskRuntime` API only and leaves
  approval/close to the restarted Master lifecycle runner
