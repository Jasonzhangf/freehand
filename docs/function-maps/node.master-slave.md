...
| 03 | `LocalNodeRuntime::pair_slave` | `crates/freehand-node/src/lib.rs` | validate websocket pairing source/ip/token and materialize paired or rejected status only after metadata admission | pairing request | paired or rejected node status | master runtime | slave runtime state | bound |
