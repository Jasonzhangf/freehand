...
| 11c | `applyPhase2QueryResult` / `refreshPhase2Status` / `renderPhase2Dashboard` / `buildMobileAgentDashboardModel` / `renderMobileAgentSummaryStrip` / `renderMobileAgentSheet` / `setMobileAgentSheetOpen` / `renderTaskBoardProjection` / `renderAgentBoardProjection` / `phase2SortedAgents` / `openWorkerTaskSession` / `renderEventInboxProjection` / `renderTaskHistoryProjection` / `renderWorkerControlProjection` / `sendWorkerControl` | `apps/freehand-server/assets/webui.js` | query TaskBoard, AgentBoard, EventInbox, TaskHistory, and WorkerControl through the existing ADP/runtime owner path; render compact read-only lifecycle observer cards; derive one mobile presentation model from owner-backed projections and selected-session turn truth; render a portrait compact Agent strip plus bottom sheet for Master evaluation and Worker review/rework/next-action/blocker/final-complete cues; keep all-children-closed as awaiting Master evaluation unless final-complete session truth exists; render active Workers first with distinct styling; click active Workers through TaskBoard truth to select their temporary worker session and refresh task history/control; route worker-control actions through `WorkerControl` and re-query owner truth after command receipt | `UiQueryResult` Phase 2 projections + selected-session turn projection + worker-control button intent + active Worker click intent + mobile sheet open/close intent | lifecycle observer cards + mobile Agent strip/sheet + selected worker child session + user-safe worker-control receipt/status text | WebUI detail drawer and mobile Agent bottom sheet | runtime owner projections through ADP | bound |
