...
- front-end mobile Agent Dashboard builds one presentation model from selected-session TaskBoard/AgentBoard truth; the Header shows running Agent count, delegated task count, and active task title, while the first tap opens only the current session's complete child-task list
...
- tapping a delegated task consumes its TaskBoard-projected `worker_session_id`, immediately refreshes `QuerySessionTurns`, and renders that Worker conversation; WebUI does not derive or guess Worker session ids
- the mobile delegated-task sheet renders every current child task for the selected parent session, not a capped slice or global history fallback; each card carries `data-task-id` and `data-worker-session-id` so online verifiers and manual inspection can check child tasks one by one
...
- WebUI mobile Agent Dashboard renders a portrait-only Header summary and delegated child-task sheet from selected-session owner truth; sheet open/close is UI-only and preserves selected session, transcript, composer draft, pending submit, scroll anchor, and lifecycle timers
...
- WebUI mobile Agent sheet renders config-owned Worker resource capacity from `QueryConfigStatus`, limits edits to `1..=5`, saves only through `UpdateAgentResourceConfig`, and never synthesizes live AgentBoard rows before restart/process startup
...
- front-end mobile tool blocks render as compact semantic rows inside assistant bubbles; status is conveyed by low-noise color background/state text and one target-focused line instead of a separate engineering card, left-edge strip, or verbose field list
...
- front-end script treats submit command timeout/transport failure as ambiguous instead of proof that the backend did nothing: it preserves the selected/draft session, pending user card, and attachment draft, marks dispatch status unknown, and tells the operator to refresh service state before sending a duplicate
...
| 11 | `refreshTurn` / `refreshAllProtocolState` / `refreshConfigStatus` / `syncSettingsProviderForm` / `submitProviderConfigUpdate` / `renderMessages` / `conversationTurnsForRender` / `buildConversationRenderModel` / `buildRenderTurn` / `buildRenderRows` / `buildToolActivityRenderRow` / `buildModelRequestRenderRow` / `turnChatCards` / `userChatBubble` / `assistantChatBubble` / `renderToolSection` / `turnIsCurrentLiveTurn` / `renderModelHasLiveLifecycle` / `logicalSessionTurns` / `isInternalRuntimePrompt` / `stripFreehandCompletionBlock` / `normalizePublicConversation` / `modelRequestKind` / `modelRequestTimingKey` / `classifyLayoutShape` / `applyLayoutShape` / `viewportDimensionsForLayout` / `setMobileDrawer` / `closeMobileDrawer` / `syncMobileDrawerForLayout` / `installMobileSessionSwipeGesture` / `setComposerFocused` / `openNewSessionDialog` / `submitNewSessionDialog` / `renderCommandStatus` / `renderSettingsShell` / `showInspectorPanel` / `refreshDebug` / `submitUserInput` / `setSessionList` / `setSessionTranscript` / `workerChildSessionsForParent` / `workerChildSessionForSessionId` / `renderSessionAgentGroup` / `renderSessionWithWorkerChildren` / `renderSessionItem` / `startNewConversation` / `startNewTask` / `renameSelectedSession` / `deleteSelectedSessions` / `rollbackLatestSessionTurn` | `apps/freehand-server/assets/webui.js` | consume ADP query/subscription turn payloads and runtime config projection, submit provider/model updates through protocol commands, manage selected and draft sessions, split global conversation creation from cwd-bound task creation through a New dialog, expose protocol-owned session rename/remove and double-Esc rollback for persisted sessions only, preserve transcript order, keep selected-session transcript pinned ahead of latest active turn during startup and manual session clicks, merge selected-session transcript with the latest same-session turn before rendering, build turn-scoped render models before DOM rendering, render chronological per-round chat bubbles without grouping same-ordinal runtime rounds into a superseded final card, map viewport width plus aspect ratio into DOM layout shape attributes without mutating protocol/session state, keep mobile portrait session/detail/settings surfaces in UI-only overlay drawers outside normal conversation flow, open the session drawer with a presentation-only content-area right-swipe gesture, render persisted sessions inside one expandable Master agent group and TaskBoard-parented worker sessions as indented temporary child rows without owning session truth, render settings from protocol/runtime config projection plus owner-backed save results without config file writes or credential values, keep mobile focused composer state as presentation-only data attributes, keep user input right-aligned, assistant/final/model rows left-aligned, embed tool rows inside assistant bubbles, hide duplicate/internal runtime continuation prompts after the first round, strip raw completion schema from assistant cards, keep same-tool updates in one semantic tool block, reset timers when typed model-request phase changes, restrict animation to the current live turn, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames + config status query result + config update receipt/failure + debug snapshot/pending state + command dispatch receipt + viewport size + mobile drawer button/settings button/gesture events | `RenderConversation` / `RenderTurn` / `RenderRow` plus settings cards/forms/status -> DOM chat bubbles + command status + layout/drawer data attributes | WebUI shell | ADP protocol frames + CSS layout rules | bound |
...
- WebUI settings shell is landed as a compact provider/model/auth-env plus configured Worker-capacity edit form; connection, active-agent, sessions/workspace, skills, files, tasks, and diagnostics cards are intentionally absent from Settings because they are status/debug projections, not editable settings
...
- WebUI submit timeout/transport failure preserves the pending conversation state and selected/draft session instead of clearing to an empty session; the browser shows an explicit unknown-dispatch status and keeps the input recoverable through history
