...
- front-end session drawer renders only protocol-persisted session summaries as top-level rows. Worker/subagent task transcripts are temporary internal sessions and may render only as indented child rows under the owning persisted master session, using TaskBoard `parent_session_id` owner truth.
