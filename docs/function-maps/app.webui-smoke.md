...
- front-end script derives visible conversation turns through one render selector that merges the selected-session transcript with the latest same-session turn, so a stale transcript cannot hide an in-flight or newly completed continuation turn
...
- front-end script renders waiting/model continuation animation only for the current live turn; historical completed turns must not keep blinking
...
| 11 | `refreshTurn` / `renderMessages` / `conversationTurnsForRender` / `logicalSessionTurns` / `isInternalRuntimePrompt` / `stripFreehandCompletionBlock` / `normalizePublicConversation` / `modelRequestKind` / `modelRequestStatusForTurn` / `modelRequestTimingKey` / `renderCommandStatus` / `refreshDebug` / `submitUserInput` / `setSessionList` / `setSessionTranscript` / `startNewConversation` / `startNewTask` | `apps/freehand-server/assets/webui.js` | consume ADP query/subscription turn payloads, manage selected and draft sessions, split global conversation creation from cwd-bound task creation, merge selected-session transcript with the latest same-session turn before rendering, render chronological per-round semantic/tool/debug cards without grouping same-ordinal runtime rounds into a superseded final card, hide duplicate/internal runtime continuation prompts after the first round, strip raw completion schema from assistant cards, keep same-tool updates in one card, reset timers when typed model-request phase changes, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames + debug snapshot/pending state + command dispatch receipt | DOM message blocks + command status | WebUI shell | ADP protocol frames | bound |
...
- WebUI selected-session transcript display now preserves chronological round cards for ids such as `runtime-turn-47` and `runtime-turn-47-r2`; the old all-rounds-in-one visible summary card is intentionally removed
- WebUI selected-session transcript rendering now uses `conversationTurnsForRender` to fold the latest same-session turn into the selected transcript, preventing stale transcript state from hiding a continued turn after user input
