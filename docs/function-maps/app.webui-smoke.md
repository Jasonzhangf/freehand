...
- front-end script uses explicit execution card color semantics: running cards are blue, successful/completed cards are green, and failed cards are red
...
| 11 | `refreshTurn` / `renderMessages` / `conversationTurnsForRender` / `buildConversationRenderModel` / `buildRenderTurn` / `buildRenderRows` / `buildToolActivityRenderRow` / `buildModelRequestRenderRow` / `turnIsCurrentLiveTurn` / `renderModelHasLiveLifecycle` / `logicalSessionTurns` / `isInternalRuntimePrompt` / `stripFreehandCompletionBlock` / `normalizePublicConversation` / `modelRequestKind` / `modelRequestTimingKey` / `renderCommandStatus` / `refreshDebug` / `submitUserInput` / `setSessionList` / `setSessionTranscript` / `startNewConversation` / `startNewTask` | `apps/freehand-server/assets/webui.js` | consume ADP query/subscription turn payloads, manage selected and draft sessions, split global conversation creation from cwd-bound task creation, preserve transcript order, merge selected-session transcript with the latest same-session turn before rendering, build turn-scoped render models before DOM rendering, render chronological per-round semantic/tool/debug cards without grouping same-ordinal runtime rounds into a superseded final card, hide duplicate/internal runtime continuation prompts after the first round, strip raw completion schema from assistant cards, keep same-tool updates in one card, reset timers when typed model-request phase changes, restrict animation to the current live turn, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames + debug snapshot/pending state + command dispatch receipt | `RenderConversation` / `RenderTurn` / `RenderRow` -> DOM message blocks + command status | WebUI shell | ADP protocol frames | bound |
