...
- usage parsing keeps Anthropic-compatible `input_tokens` as the provider-reported field, writes the contracts-resolved total as explicit `normalized_input_tokens`, and projects cache hit rate and total tokens from that normalized total without inflating cache counters already contained in provider input
...
| 02b | `parse_anthropic_usage` | `crates/freehand-provider-anthropic/src/lib.rs` | preserve provider-reported input and write the contracts-resolved total as explicit normalized-input usage | raw Anthropic usage object | provider-neutral TokenUsage | AnthropicAdapter::parse_response / AnthropicAdapter::parse_stream_event | adapter usage parser |  |  |  | bound |
