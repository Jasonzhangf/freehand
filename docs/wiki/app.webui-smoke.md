...
| 11 | `refreshTurn / refreshSessions / renderMessages / logicalSessionTurns / stripFreehandCompletionBlock / normalizePublicConversation / classifyLayoutShape / applyLayoutShape / renderCommandStatus / refreshDebug / submitUserInput / setSessionList / setArchivedSessionList / renameSelectedSession / restoreArchivedSession / rollbackLatestSessionTurn` | `apps/freehand-server/assets/webui.js` | consume ADP query and subscription turn payloads, render semantic/tool/debug cards, expose protocol-owned session rename/archive/archived-list/restore/rollback controls, keep same-tool updates in one card, apply aspect-ratio layout shape attributes, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames plus debug snapshot/pending state plus command dispatch receipt plus selected session UI intent plus viewport size | DOM message blocks, session lists, archived list, composer refill after rollback receipt, command status, and layout shape attributes | WebUI shell | ADP protocol frames | bound |
