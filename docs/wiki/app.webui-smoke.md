...
| 11 | `refreshTurn / refreshSessions / renderMessages / logicalSessionTurns / stripFreehandCompletionBlock / normalizePublicConversation / classifyLayoutShape / applyLayoutShape / viewportDimensionsForLayout / setMobileDrawer / closeMobileDrawer / syncMobileDrawerForLayout / renderCommandStatus / refreshDebug / submitUserInput / setSessionList / setArchivedSessionList / renameSelectedSession / restoreArchivedSession / rollbackLatestSessionTurn` | `apps/freehand-server/assets/webui.js` | consume ADP query and subscription turn payloads, render semantic/tool/debug cards, expose protocol-owned session rename/archive/archived-list/restore/rollback controls, keep same-tool updates in one card, apply aspect-ratio layout shape attributes, keep mobile portrait session/detail surfaces in UI-only overlay drawers outside normal conversation flow, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames plus debug snapshot/pending state plus command dispatch receipt plus selected session UI intent plus viewport size plus mobile drawer button events | DOM message blocks, session lists, archived list, composer refill after rollback receipt, command status, layout shape attributes, and mobile drawer state attributes | WebUI shell | ADP protocol frames | bound |
