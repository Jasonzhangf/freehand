...
- WebUI provider/model save renders field-level validation errors, disables duplicate save while in flight, re-queries owner-backed config status, and shows restart-required after a successful owner-backed save
...
| 11 | `refreshTurn / refreshSessions / refreshConfigStatus / syncSettingsProviderForm / submitProviderConfigUpdate / renderMessages / logicalSessionTurns / stripFreehandCompletionBlock / normalizePublicConversation / classifyLayoutShape / applyLayoutShape / viewportDimensionsForLayout / setMobileDrawer / closeMobileDrawer / syncMobileDrawerForLayout / installMobileSessionSwipeGesture / setComposerFocused / openNewSessionDialog / submitNewSessionDialog / renderCommandStatus / refreshDebug / submitUserInput / setSessionList / workerChildSessionsForParent / workerChildSessionForSessionId / renderSessionAgentGroup / renderSessionWithWorkerChildren / renderSessionItem / renameSelectedSession / deleteSelectedSessions / rollbackLatestSessionTurn` | `apps/freehand-server/assets/webui.js` | consume ADP query and subscription turn payloads, render semantic/tool/debug cards, submit provider/model updates through protocol commands, expose protocol-owned session rename/remove/rollback controls, open the New dialog for CreateSession-backed global conversation or cwd-bound task creation, keep same-tool updates in one card, apply aspect-ratio layout shape attributes, keep mobile portrait session/detail surfaces in UI-only overlay drawers outside normal conversation flow, open the session drawer with a presentation-only content-area right-swipe gesture, render persisted sessions inside one expandable Master agent group and TaskBoard-parented worker sessions as indented temporary child rows without owning session truth, keep mobile focused composer state presentation-only, and render settings from protocol/runtime config projection plus owner-backed save results without config file writes or credential values, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames plus debug snapshot/pending state plus config status/update frames plus command dispatch receipt plus selected session UI intent plus viewport size plus mobile drawer button and gesture events | DOM message blocks, settings cards/forms/status, persisted session lists with indented worker child rows, New dialog state, composer refill after rollback receipt, command status, layout shape attributes, composer focus attributes, and mobile drawer state attributes | WebUI shell | ADP protocol frames |  |  |  | bound |
...
- WebUI settings provider/model save submits UpdateProviderConfig over ADP, shows validation errors or restart-required success, re-queries owner-backed config projection, and avoids raw API-key values/direct config writes
