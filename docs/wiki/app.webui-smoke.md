...
- front-end session rail exposes protocol-owned rename, archive, archived-session list, restore, and double-Esc rollback controls without storing session truth locally
...
| 11 | `refreshTurn / refreshSessions / renderMessages / logicalSessionTurns / stripFreehandCompletionBlock / normalizePublicConversation / classifyLayoutShape / applyLayoutShape / viewportDimensionsForLayout / setMobileDrawer / closeMobileDrawer / syncMobileDrawerForLayout / installMobileSessionSwipeGesture / setComposerFocused / openNewSessionDialog / submitNewSessionDialog / renderCommandStatus / refreshDebug / submitUserInput / setSessionList / sessionAgentId / groupedSessionsByAgent / renderSessionAgentGroup / renderSessionItem / renameSelectedSession / deleteSelectedSessions / rollbackLatestSessionTurn` | `apps/freehand-server/assets/webui.js` | consume ADP query and subscription turn payloads, render semantic/tool/debug cards, expose protocol-owned session rename/remove/rollback controls, open the New dialog for global conversation or cwd-bound task creation, keep same-tool updates in one card, apply aspect-ratio layout shape attributes, keep mobile portrait session/detail surfaces in UI-only overlay drawers outside normal conversation flow, open the session drawer with a presentation-only content-area right-swipe gesture, render session summaries as agent -> sessions groups without owning session truth, keep mobile focused composer state presentation-only, and keep missing debug snapshots pending instead of failed | `UiAdpResponse` frames plus debug snapshot/pending state plus command dispatch receipt plus selected session UI intent plus viewport size plus mobile drawer button and gesture events | DOM message blocks, agent-grouped session lists, New dialog state, composer refill after rollback receipt, command status, layout shape attributes, composer focus attributes, and mobile drawer state attributes | WebUI shell | ADP protocol frames | bound |
...
- WebUI session rail now exposes rename, archive, archived-session list, restore, and double-Esc rollback as protocol commands over ADP instead of local session truth
