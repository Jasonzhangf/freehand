...
- WebUI provider registry renders configured providers, provider definition save and provider selection switch render field-level validation errors, disable duplicate actions while in flight, re-query owner-backed config status, preserve owner fallback selection until an explicit selector edit, and show restart-required after a successful owner-backed save
...
| 02 | `assets::asset_response` | `apps/freehand-server/src/assets.rs` | serve version-addressed split CSS and JS assets with explicit content type and no-store cache policy so Android WebView cannot retain stale dashboard code | asset path | CSS or JS response or 404 | app asset route | embedded assets |  |  |  | bound |
...
- WebUI selected-session transcript refresh renders nonterminal protocol model_request rows with static waiting/retrying/switching status when they are not current live, terminal truth overrides outstanding submit activity, and scripts/verify-webui-live-tool-render-online.mjs scopes fixed-session evidence to the current run marker and newly added base/continuation turn ids while proving live tool cards plus continuation waiting before final summary and zero live rows after current-run terminal success
