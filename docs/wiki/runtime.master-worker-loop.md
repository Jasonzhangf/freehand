...
- before reading Task Center events, the Master lifecycle runner claims due independent timer schedules and resumes the Master with the persisted wakeup prompt
...
| 11 | `ProductionMasterRunner::run_once` | `crates/freehand-runtime/src/master_runner.rs` | claim due timer wakeups before draining new Task Center events from the durable lifecycle cursor | timer store plus Task Center and cursor | timer-fired, idle, or task decision outcome | Master lifecycle service and tests | ProductionMasterRunner::handle_due_timer, TaskRuntime::query_event_inbox, and ProductionMasterRunner::handle_event |  |  |  | bound |
