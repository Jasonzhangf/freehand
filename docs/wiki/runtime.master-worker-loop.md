...
- a resolved busy-Master attention stores typed decision kind and changed task ids in master_work and restores the original work_id, session_id, logical turn id, and trace id without raw Worker/control transcripts or provider payloads
...
- busy-Master active-work identity, lower-priority deferral, safe-point high-priority suspension state, typed attention resolution, and exact return identity are unit-bound; live continuation injection remains pending
