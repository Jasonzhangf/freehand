...
- decided blocked-child follow-up truth is durable-idempotent by parent session, logical parent turn, blocked task id, and Master blocked_decision event sequence; EventInbox replay or daemon restart cannot duplicate the same parent blocked follow-up turn
...
| 11 | `ProductionMasterRunner::run_once` | `crates/freehand-runtime/src/master_runner.rs` | repair stale EventInbox cursor if needed, admit Task Center events in source order, dequeue pending attention by deterministic weighted aging, preserve retry identity, drain stale no-ops, reconcile decided-blocked and closed parent worksets whose parent turn still waits for lifecycle, then run due timer wakeups only when attention is empty | Task Center cursor plus pending attention plus timer store | task decision, blocked parent follow-up, parent-evaluation skip/evaluation, timer-fired, or idle outcome | Master lifecycle service and tests | ProductionMasterRunner::query_event_inbox_repairing_stale_cursor, ProductionMasterRunner::admit_attention_events, highest_priority_attention_index, ProductionMasterRunner::handle_event, ProductionMasterRunner::reconcile_blocked_parent_worksets, ProductionMasterRunner::reconcile_closed_parent_worksets, and ProductionMasterRunner::handle_due_timer |  |  |  | bound |
