...
- per-turn usage is projected as UiUsageProjection inside UiTurnProjection.usage_projection with input/output/reasoning/cache tokens, cache-hit-rate bps, context tokens, and compacted-token counters; cache-hit-rate bps is cache-read tokens over total input tokens, and protocol exposes the projection while provider usage truth stays reason-owned
