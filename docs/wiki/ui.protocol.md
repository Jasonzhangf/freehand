...
- public conversation tool summaries carry tool_call_id so UI clients can update one tool card instead of rendering duplicate waiting/completed cards, and public tool bodies are status-only by default
...
- public tool summaries preserve tool_call_id, duplicate same-id tool calls upsert into one public card, and the default public tool body stays status-only
