...
| 07 | `data_tunnel / attach_data` | `crates/freehand-relay/src/websocket_tunnel.rs` | admit one data channel only after control admission | authenticated Agent identity | typed data tunnel | data_tunnel | attach_data | relay_control_tunnel | relay_data_tunnel | relay_control_tunnel.admit_data | bound |
...
| 11 | `run_error_socket / RelayTunnelRegistry::fail_exchange` | `crates/freehand-relay/src/websocket_tunnel.rs / crates/freehand-relay/src/tunnel.rs` | validate typed Agent tunnel failures, remove only the authenticated correlated exchange, deliver its exact failure, terminate malformed error channels explicitly, and generation-fence stale error-socket cleanup | typed Agent error frame plus authenticated tunnel identity | correlated pending-exchange failure or explicit terminal error-channel failure | run_error_socket | RelayTunnelRegistry::fail_exchange | relay_error_tunnel | relay_data_tunnel | relay_error_tunnel.correlate | bound |
...
| 13 | `agent_tunnel_config_from_env / RelayAgentClient::run` | `apps/freehand-relay-server/src/main.rs / crates/freehand-relay/src/agent_client.rs` | load explicit Agent bridge configuration and enter the typed outbound tunnel lifecycle | Agent deployment environment | live control, data, and error Agent tunnels | agent_tunnel_config_from_env | RelayAgentClient::run | relay_control_tunnel | relay_control_tunnel | relay_control_tunnel.connect | bound |
| 14 | `attach_error / RelayTunnelRegistry::admit_error` | `crates/freehand-relay/src/websocket_tunnel.rs / crates/freehand-relay/src/tunnel.rs` | atomically admit one generation-fenced error channel only while the same authenticated control identity remains attached | authenticated Agent identity and typed error sender | control-owned typed error tunnel generation | attach_error | RelayTunnelRegistry::admit_error | relay_control_tunnel | relay_error_tunnel | relay_control_tunnel.admit_error | bound |
...
- product config and Master daemon outbound-client wiring are bound outside this module; Worker UI/ADP hosting, dynamic heartbeat projection, Agent Dashboard, and Android login remain incomplete
