...
- `AgentLifecycleReducer`
  ...
  - related tests: model_thinking reducer test, tool_running reducer test, recovering reducer test, blocked reducer test
...
- `AgentBoardProjection`
  ...
  - related tests: AgentBoard query test, restart same-id proof
...
| 03 | `AgentBoardProjection` | `crates/freehand-task/src/lib.rs` | project all agent lifecycle snapshots for master, scheduler, UI, and headless query | lifecycle state map | AgentBoard projection | lifecycle owner | runtime query dispatch | pending |
