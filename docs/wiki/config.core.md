...
- account_config_document.apply_shared_account_config
...
- account-shared document apply requests enter only through config.core::apply_shared_account_config; the config owner validates shared provider/model-group/relay/daemon ids and references, compiles an in-memory effective LoadedConfig, and never rewrites the local canonical config
