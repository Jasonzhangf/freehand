...
- release script runs full regression, release binary builds, Android JVM regression, Android release APK packaging with Android release lint disabled in Gradle config, extracts APK version truth, and stages deterministic artifacts including dist/android/update.json
...
- current gate baseline enforces required files, policy docs, generated wiki freshness, feature-map seed-entry uniqueness, resource-map ownership/relation/source-edge/test-coverage consistency, migrated mainline manifest cross-links, migrated mainline call-table bindings, CI/CD full-gate command alignment, source-search boundaries, and static data/control boundary checks
