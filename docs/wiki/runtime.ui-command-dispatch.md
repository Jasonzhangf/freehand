...
| 31 | `RuntimeCommandDispatcher::dispatch_compact_session_context` | `crates/freehand-runtime/src/lib.rs` | route CompactSessionContext through reason.rewrite-policy truth by restoring the persisted session history and running ReasonRewriteRuntime::apply_compaction_policy, returning an explicit hold/soft-notice/stale-prune/staged receipt without fabricating a rewrite result; missing persisted recovery truth fails dispatch explicitly | CompactSessionContext command plus request reason | compaction policy receipt with explicit hold/soft-notice/stale-prune/staged status or explicit dispatch failure | ADP command transport | ReasonPersistence::restore + ReasonRewriteRuntime::apply_compaction_policy | session | ui_projection | session.restore | bound |
...
- runtime CompactSessionContext dispatch bridge routes the protocol-owned mutation command to reason.rewrite-policy truth via session.restore plus apply_compaction_policy and returns an explicit hold/stale/soft/staged receipt, never fabricating a rewrite result; missing recovery truth fails dispatch explicitly; covered by runtime_compact_session_context_hold_when_no_provider_summary_payload and runtime_compact_session_context_rejects_missing_recovery_truth_explicitly
