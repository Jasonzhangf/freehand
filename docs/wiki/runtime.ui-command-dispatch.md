...
- `QuerySessionTurns` enters through RuntimeCommandDispatcher::query_runtime, restores effective logical-turn snapshots from reason.persistence, hides framework-owned parent-evaluation and worker-task prompts from user_text, and replaces the derived session transcript so background-written turns are visible without daemon restart while preserving same-turn nonterminal live provider/model waiting and tool activity already published into UiProtocolState
...
- runtime read-only config status queries reload config-owner truth from the runtime home and project the selected live agent/provider config plus complete safe provider registry without exposing API keys, pair tokens, or credential-bearing URLs
...
- active live cancel requests set the active cancel token immediately and publish a cancelled UI projection without waiting for provider completion; cancellation or pre-provider failure closes the same prepared active snapshot instead of creating a replacement turn, and framework-owned worker-task cancelled prompts remain hidden from user_text
...
- runtime-backed QuerySessionTurns refreshes the requested session transcript from reason persistence truth at query time; every runtime-turn-N / runtime-turn-N-rM round remains chronological and retains its own tool activity, active same-turn provider retry/model waiting and tool activity already in UiProtocolState are preserved for nonterminal refreshes, parent-goal evaluation internal prompt turns hide their synthetic user text while preserving the final assistant decision, and worker-task transcripts hide internal task/continuation prompts while preserving Worker source-agent attribution and final/tool truth
...
- runtime config status queries return UI-safe projections sourced from config.core selected-agent truth with the ordered peer name/mode/node-id/provider list, complete provider registry, current primary/fallback provider ids, resource count, resource limit, shared provider id, and auth source type only
...
- `project_config_status_for_ui`
  - owner: `crates/freehand-runtime/src/lib.rs`
  - purpose: build UI-safe active or pending config projection plus complete provider registry from config.core selected agent truth
...
| 10q | `RuntimeCommandDispatcher::query_runtime` | `crates/freehand-runtime/src/lib.rs` | execute runtime-backed read-only session-turn, task list/history, config, and error-center queries | UI query command | optional UI query result or dispatch failure | ADP query transport | reason persistence / task runtime owner / config projection / metadata center |  |  |  | bound |
| 10s | `RuntimeCommandDispatcher::query_runtime / restore_session_turns_for_ui_query / UiProtocolState::replace_session_turn_projections` | `crates/freehand-runtime/src/lib.rs` | restore exact-round QuerySessionTurns snapshots from the master plus configured Worker reason-persistence namespaces and replace the derived transcript at query time without projecting framework-owned parent/Worker prompts as user-authored text or erasing same-turn nonterminal live provider/tool activity | session id | fresh queryable transcript with runtime-turn-N / runtime-turn-N-rM rounds preserved, per-round tool activity visible, active live provider/tool activity preserved on same-turn nonterminal refresh, internal parent-goal evaluation user text hidden, and Worker task transcript rows sourced to the configured Worker with internal task prompts hidden from user_text | ADP query transport / runtime query tests | ReasonPersistence::restore_turn_snapshots_for_ui + configured agent/node map + turn_projection + ui.protocol state |  |  |  | bound |
...
| 15 | `UiProtocolState::replace_session_turn_projections` | `crates/freehand-ui-protocol/src/lib.rs` | replace one session transcript with persistence-owned effective projections after rollback or selected-session refresh while preserving same-turn nonterminal live provider/tool activity | session id plus effective turn projections | queryable transcript excluding rolled-back logical turns and preserving active live activity until terminal truth arrives | RuntimeCommandDispatcher::dispatch_session_management | ui.protocol state |  |  |  | bound |
...
- selected QuerySessionTurns refresh preserves active same-turn provider retry/model waiting and tool activity already published into UiProtocolState
