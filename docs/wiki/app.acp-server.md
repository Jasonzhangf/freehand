...
- master_work.project_acp_broadcast
...
- session/prompt streams runtime reasoning, message, tool, and tool-result events as ACP session/update notifications via the AcpBroadcaster projection; usage events are not projected until the reason.persistence owner exposes a typed context-window projection
...
| 02 | `run_turn_blocking` | `crates/freehand-acp/src/lib.rs` | load the selected agent config and run one live-reason turn with the session cancel token on the tokio blocking pool, streaming broadcast events through hooks | ACP sessionId plus prompt text | LiveReasonTurnOutcome observed under cancel | FreehandAgent::connect_to prompt handler | run_live_reason_turn_with_hooks | acp_transport | master_work | acp_transport.prompt_turn | bound |
...
| 05 | `AcpBroadcaster::on_broadcast / project_broadcast` | `crates/freehand-acp/src/lib.rs` | project runtime broadcast events (semantic, tool, tool-result) into ACP session/update notifications streamed to the client | ReasonBroadcastEvent stream | ACP session/update notifications | run_turn_blocking | project_semantic / project_tool_call / project_tool_result / project_usage | master_work | acp_transport | master_work.project_acp_broadcast | bound |
...
- capability advertisement (initialize) declares only the methods and prompt content kinds that the adapter actually handles; authenticate/session/list/session/load/session/resume/session/close are not advertised and are out of scope for this slice
- session/new records the session working directory; session/prompt forwards it to LiveReasonTurnRequest.cwd
- session/prompt streams runtime reasoning/message/tool/tool-result events as ACP session/update notifications via AcpBroadcaster; usage events are not projected because the runtime lacks a provider context-window projection
- ACP tool kind projection goes through the tool.display owner via freehand-runtime's typed port; ACP does not re-classify tool names itself
