...
- for ADP turn samples, CLI connects to the same daemon `/adp`, subscribes to latest-turn updates, submits a success or failure sample prompt, and verifies the matching terminal projection
...
- ADP turn samples print the observed command outcome plus the matching Success or Failed latest-turn projection, giving WebUI and automation a reproducible positive and negative sample
...
- ADP turn sample timeout or wrong terminal status returns explicit terminal errors
...
| 09 | `run_adp_turn_sample_async` | `apps/freehand-cli/src/main.rs` | submit success/failure sample prompts over ADP and verify matching terminal projection | ADP WebSocket URL plus sample kind | observed success or failed turn projection | ADP sample runner | daemon /adp | bound |
...
- CLI ADP success/failure turn sample path is implemented
