...
- CLI Phase 1/2B foundation samples that send internal scheduler/worker commands over /adp require matching server FREEHAND_ADP_INTERNAL_COMMAND_TOKEN plus explicit CLI FREEHAND_ADP_SEND_INTERNAL_COMMAND_TOKEN=1 before the CLI sends its local token, otherwise fail explicitly with adp_command_not_public
...
| 09j | `run_reason_live` | `apps/freehand-cli/src/main.rs` | run a real configured provider through the production live bridge with explicit provider, Worker role, and sourced-search profile selection | natural-language prompt plus configured provider id and sourced_search execution profile | runtime-owned schema rejection, delivery, verified-source, claim-binding, validator, and terminal metrics | operator/agent verifier | run_worker_live_reason_turn or run_live_reason_turn |  |  |  | bound |
...
| 09d | `run_phase1_foundation_sample / run_phase1_foundation_sample_async / run_phase1_foundation_verify_async` | `apps/freehand-cli/src/main.rs` | drive Phase 1 TaskBoard, AgentBoard, ExecutionFact, SchedulerTick, and same-id restart verification through ADP | ADP WebSocket URL plus optional verify ids plus ADP handshake | terminal-facing Phase 1 foundation evidence or explicit ADP/query failure | CLI dispatcher using ADP client | daemon /adp public query/command path for public commands plus server-token-gated internal harness command path |  |  |  | bound |
