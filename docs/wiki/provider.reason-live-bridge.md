...
- bridge derives provider descriptor and executor config from selected provider truth; Anthropic output budget stays at the adapter default `DEFAULT_ANTHROPIC_MAX_TOKENS=8192` without a smaller live-bridge cap
...
- provider execution failures are classified with concrete error codes and retried up to ten non-stream attempts with exponential backoff starting at 1 second before failed terminal truth is materialized
...
- provider executor and transport failures are distinct from schema mismatch polishing and tool execution result failures: recoverable non-stream failures retry up to ten attempts, then materialize a failed terminal turn with a concrete provider error code and no active turn before the dispatch error is returned
