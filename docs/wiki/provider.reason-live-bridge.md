...
- live context segment construction is owned by crates/freehand-runtime/src/live_context.rs and consumed by the src/lib.rs live bridge orchestrator through base_live_context_segments/base_live_context_segments_with_observer; Master TaskSpaceSnapshot content is sourced from task owner TaskRuntime::query_task_space_snapshot as a bounded read-only projection rather than by booting full TaskRuntime from the live context path
- path and symlink diagnostics are owned by crates/freehand-runtime/src/path_diagnostics.rs and shared by Worker target-cwd preflight plus Master task-tool target-cwd diagnostic text
...
- sourced-search hosted/camo/social failure enters exactly one recovery round whose function-tool surface is only concrete-URL `web_fetch`; successful recovery is a discovery candidate that still requires camo verification, while failed or unusable recovery immediately writes Blocked
...
- completed provider tool calls are classified by registry execution scope; Master local workspace read/search/write/edit tools execute only in the current selected session cwd under tool.registry path and checkpoint locks, Master concrete-URL `web_fetch` executes through the tool registry network scope, and Master `task` and `timer` remain framework tools; injected Master shell, browser, broad web_search, todo, or complete-step calls return paired failed capability-boundary guidance with no file-content leak; Worker read/search/write path tools remain locked to the task cwd after absolute-normalization and symlink/canonical resolution, Worker `web_fetch` executes only concrete HTTP/HTTPS URL reads, and Worker boundary failures instruct relative in-cwd paths or blocked return instead of external probing; Worker shell/unknown-tool failures include the exact Worker tool list and forbid invented `shell`/`readlink` style calls; incomplete `tool_use` calls and execution failures become paired failed tool-result exchanges, while writable in-root tool calls first go through runtime checkpoint preview/snapshot/execute gating before success or execution-failure results are written back through `ReasonTurnEngine::apply_provider_output`, persisted, and sent to the next Anthropic request
- runtime writes tool execution lifecycle metadata without tool-result content before tool-result re-entry; failed tool-result error-center metadata stores only owner/error classification, while the full model-visible recovery text stays in reason tool-result truth
...
- schema retry exhaustion closes with provider terminal truth when present through `ReasonTurnEngine::block_turn`; provider interruption without a completion-schema candidate writes interrupted terminal truth through `ReasonTurnEngine::interrupt_turn`
- sourced-search hosted/camo/social evidence exhaustion attempts one `web_fetch` recovery before Blocked; no extra provider round follows a failed or unusable recovery
...
| 05a | `write_live_bridge_metadata` | `crates/freehand-runtime/src/lib.rs` | write runtime-owned context-planning and context-segment lifecycle metadata without request text | role plus stream mode plus cwd binding plus configured worker count plus context segment id/status/inclusion/elapsed facts plus final segment count and estimated token budget | durable runtime metadata record | live bridge | metadata owner |  |  |  | bound |
| 05b | `emit_live_bridge_debug` | `crates/freehand-runtime/src/lib.rs` | emit runtime-owned context-planning and context-segment lifecycle debug snapshots without request text | context planning started/completed lifecycle facts plus segment started/completed/failed facts | runtime-owned debug event consumable as model-waiting UI activity | live bridge | debug.core |  |  |  | bound |
| 05c | `live_context::base_live_context_segments / live_context::base_live_context_segments_with_observer` | `crates/freehand-runtime/src/live_context.rs` | build typed completion, control, tool-guidance, instruction-capability, bounded task-space, and original-task context segments outside the live bridge orchestrator, with optional owner-safe segment build events | original prompt plus role plus configured Worker set plus runtime home plus cwd plus agent id | ordered base live context segments plus optional started/completed/failed segment observations, or explicit owner error | run_live_provider_reason_turn and next_round_segments | context, instruction, and task owners |  |  |  | bound |
...
| 11 | `build_live_provider_driver / CompositeProviderExecutorFactory::build_executor` | `crates/freehand-runtime/src/lib.rs / crates/freehand-provider-executors/src/lib.rs` | select a provider-core live executor from config through the provider executor assembly without runtime depending on concrete adapter crates | selected provider type/protocol plus descriptor/auth/base URL | boxed `ProviderLiveExecutor` or explicit unsupported/build error | live bridge | provider-core executor factory assembly |  |  |  | bound |
| 12 | `ProviderLiveExecutor::execute_once_with_raw / ProviderLiveExecutor::execute_stream_with_raw` | `crates/freehand-provider-core/src/lib.rs` | execute the provider-selected request through one provider-core trait while provider adapter crates own protocol-specific wire rendering/parsing | provider semantic request plus provider-neutral raw/output callbacks | incremental raw event bodies plus incremental semantic output batches plus accumulated outputs | live bridge | provider-core executor trait |  |  |  | bound |
...
| 27 | `run_live_provider_reason_turn / provider_executor_retry_plan / sleep_provider_retry / materialize_provider_executor_failure` | `crates/freehand-runtime/src/lib.rs` | retry recoverable non-stream primary failures up to ten attempts, wait between retries through cancel-token-aware backoff, switch once to the configured fallback after eligible primary exhaustion at the provider-neutral request boundary, and convert inapplicable or exhausted fallback failure into one runtime-classified failed turn | provider executor error plus active route plus optional fallback plus retry plan plus active turn plus cancel token | retry continuation, interruptible retry wait, explicit route-switch metadata plus fallback semantic request, or persisted failed/cancelled closed turn with concrete provider error code | live bridge executor error path | config, error, reason, and persistence owners |  |  |  | bound |
...
- runtime live bridge now emits context segment started/completed/failed metadata and debug snapshots for completion, control, tool-guidance, instruction-capability, task-space, and original-task segment construction without prompt or provider-payload leakage
...
- provider executor and transport failures are distinct from schema mismatch polishing and tool execution result failures: recoverable non-stream primary failures retry up to ten attempts through cancel-token-aware backoff and then switch once to the configured fallback; fallback exhaustion materializes one failed terminal turn with a concrete provider error code and no active turn before dispatch error return
