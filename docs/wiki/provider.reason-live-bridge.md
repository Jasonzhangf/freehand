...
- sourced-search hosted/camo/social failure enters exactly one recovery round whose function-tool surface is only concrete-URL `web_fetch`; successful recovery is a discovery candidate that still requires camo verification, while failed or unusable recovery enters at most two blocked-finalization rounds before explicit Blocked
...
- sourced-search hosted/camo/social evidence exhaustion attempts one `web_fetch` recovery, then allows at most two blocked-finalization rounds before Blocked; no unbounded provider recovery loop is allowed
