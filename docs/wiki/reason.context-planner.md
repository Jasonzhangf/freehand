...
- attention resolution segments are turn-volatile/no-cache developer context, ordered after refreshed task-space snapshots, and carry only typed changed truth plus resume identity for the original foreground turn
...
- attention resolutions are rejected from rewrite-base gates because they are one-turn changed-truth signals, not stable session memory
...
- current baseline includes first-class `TaskContract`, `TaskSpaceSnapshot`, and `AttentionResolution` segment kinds with cache-shape, ordering, and rewrite-base coverage
...
- runtime-owned dynamic input segments such as operator task and previous visible output use content-derived admission budgets before planner validation
