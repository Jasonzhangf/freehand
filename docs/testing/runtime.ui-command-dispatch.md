...
| `session.list_persisted` | bound | `cargo test -p freehand-runtime runtime_query_session_search_returns_worker_hits_under_parent_session -- --nocapture --test-threads=1` verifies runtime Search consumes persisted index/metadata truth and keeps Worker matches nested under the parent | `cargo test -p freehand-ui-protocol session_search -- --nocapture --test-threads=1` verifies route separation and DTO validation while persistence remains owner truth | `node scripts/verify-webui-session-search-online.mjs` proves browser Search queries owner-backed persisted session truth and does not create/promote top-level Worker sessions |
| `session.list_persisted_page` | bound | `cargo test -p freehand-runtime runtime_session_list_page_avoids_full_transcript_restore -- --nocapture --test-threads=1` verifies the runtime bridge maps latest paging and does not restore a poisoned session's full transcript to list other rows | `cargo test -p freehand-reason session_list_page -- --nocapture --test-threads=1` verifies reason-owned ordering, cursor, metadata-only summaries, archive separation, migration, and unavailable facts | `node scripts/verify-webui-mobile-ui-tree-online.mjs` proves production WebUI renders Home session history from protocol projections; S-profile 75-session proof remains required below |
...
- lifecycle path under test:
  ...
  - runtime-backed `QuerySessionListPage` returns one metadata-only public page, scans across internal-only persistence pages without projecting an empty UI page, anchors Older cursors to visible sessions, and never performs full transcript restoration merely because an unavailable/poisoned summary exists
    - positive/negative: `runtime_session_list_page_skips_internal_only_pages_without_empty_ui_page` proves 120 internal rows do not create an empty public page and that the Older cursor resumes at the next visible user session instead of skipping it
    - negative: `runtime_rejects_invalid_session_list_cursor_without_projection` proves malformed Older cursor input enters `UiCommandDispatchPortError::DispatchFailed` and never becomes a successful page projection
