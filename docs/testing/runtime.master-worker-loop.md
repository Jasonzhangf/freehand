...
| `agent.project_runtime_agent_activity` | bound | `cargo test -p freehand-runtime production_worker_runner_host_cancellation -- --nocapture` verifies Worker lifecycle state is read and cancellation closes active execution explicitly; `cargo test -p freehand-runtime production_master_runner_activity -- --nocapture` verifies idle, actionable background work, retry wait, terminal error, and completed-work reset from the Master runner owner | `cargo test -p freehand-daemon relay_presence_projection_maps_only_typed_control_activity -- --nocapture` verifies foreground dispatcher and background runner activity merge before typed Relay mapping | `scripts/verify-remote-relay-local-online.sh` plus authenticated Relay directory observation verifies online Master/Worker status changes and never treats a stopped source as idle success |
...
11. Relay presence reads Worker activity from AgentLifecycle truth only: active
    execution states project Running, blocked/review/recovery project Waiting,
    failed projects Error, and terminal/idle state projects Idle.
12. Relay presence reads background Master lifecycle activity from
    `ProductionMasterRunner` owner state. Actionable task, timer, and parent
    evaluation execution projects Running; retry delay projects Waiting;
    terminal runner failure projects Error. Idle scans and completed/cancelled
    work project Idle, and terminal Error cannot be overwritten by work cleanup.
13. Restart reads the same task/execution/agent/history truth and increments
    process restart count only for a new process-instance identity.
13. Each Master lifecycle event attempt runs in a task-scoped internal
...
| Bootstrap stale user-session wait | reason closed-turn truth, TaskBoard parent links, TimerStore source schedules, live `master_work` checkpoint | legacy latest effective `ToolPending` with no active turn, no model/tool request, no open child task, no active/running source timer, and no live Master work owner | live runtime bootstrap must rewrite the same latest turn to terminal `Blocked`; wakeable child-task/timer/live-master owners must preserve `ToolPending` | SessionList latest status, restored turn terminal truth, absence/presence of owner-backed wait | `cargo test -p freehand-runtime live_bootstrap_closes_stale_toolpending_without_lifecycle_owner`, `live_bootstrap_tolerates_incomplete_authoritative_history_with_empty_ledger -- --nocapture --test-threads=1`; `cargo test -p freehand-runtime live_bootstrap_keeps_toolpending_when_child_task_can_wake_parent -- --nocapture --test-threads=1`; `node scripts/verify-webui-mobile-ui-tree-online.mjs` |
| Master runner activity projection | Master loop pending attention, due timer, parent workset, retry policy, and terminal runner result | idle bootstrap/scan, one actionable owner execution, retryable execution failure, non-retryable owner-truth failure, and already-completed workset | only actionable execution is `Running/1`; retry backoff is `Waiting/0`; terminal failure is `Error/0`; idle, cancellation, and completed work are `Idle/0` | `ProductionMasterRunner::current_agent_activity` typed control projection only | `cargo test -p freehand-runtime production_master_runner_activity -- --nocapture` |
| Worker assigned activity projection | Worker AgentLifecycle state plus current task binding | idle Worker and assigned-but-not-yet-claimed task | idle remains `Idle/0`; Assigned projects as `Waiting/1` rather than a false idle state | `ProductionWorkerRunner::current_agent_activity` typed Relay control projection only | `cargo test -p freehand-runtime production_worker_runner_projects_assigned_task_as_waiting_activity -- --nocapture` |
| Worker control and execution visibility | Task Center WorkerControl ledger plus running execution fact | queued Master question/constraint/checkpoint/submission and an active Worker model request; external cancel racing a model result | queued controls are present in the next request, `TaskExecutionRecorded` precedes provider IO, and applied cancel yields idle without review/block overwrite | `worker_control_prompt_context`, `TaskRuntime::record_execution`, and cancel-aware Worker runner path | `cargo test -p freehand-runtime worker_runner -- --nocapture --test-threads=1` |
...
- Live runtime bootstrap closes a persisted user-session `ToolPending` latest
  turn as `Blocked` when no owner truth can wake it after restart, and preserves
  `ToolPending` when an open same-session child task can still wake the parent:
  `live_bootstrap_closes_stale_toolpending_without_lifecycle_owner`,
  `live_bootstrap_keeps_toolpending_when_child_task_can_wake_parent`.
...
- Worker host cancellation during an active provider turn trips the same live
  cancel token and persists `TaskInterrupted` before the runner lifetime
  closes; daemon/tunnel shutdown must not leave the task in `Running` or wrap
  the interrupted execution as idle success.
...
- A legacy `ToolPending` latest turn with no active turn and no TaskBoard,
  TimerStore, or live Master work owner must not survive bootstrap as running
  lifecycle truth; it is closed as `Blocked` before SessionList projection.
- An owner-backed `ToolPending` with an open child task must not be closed by
  bootstrap cleanup; premature cleanup would break valid Master/Worker waits.
...
- A completed blocked-follow-up marker must not be trusted after rollback
  removes the matching follow-up from effective parent transcript truth; startup
  reconciliation must clear that stale marker and reserve a fresh turn id.
...
- live bootstrap stale-wait coverage proves `RuntimeCommandDispatcher::new`
  reconciles reason closed-turn truth before UI projection restore: no-owner
  stale `ToolPending` becomes `Blocked`, while an open child TaskBoard owner
  preserves the parent wait.
