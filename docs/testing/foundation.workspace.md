...
- white-box plan:
  ...
  - repository evidence binds an attestation to a distinct raw report under `docs/migrations/openminis-ui/evidence/` by canonical path and SHA-256 and validates its schema, run id, command, terminal result, assertions, and attested source revision; WebUI, Android-device, and legacy no-touch reports fail closed until a non-repository, source-bound external provenance verifier exists, so coordinated hand-written success report/artifact/digest changes cannot promote lifecycle truth
...
- OpenMinis positive/negative test matrix:
  ...
  - positive/negative evidence: complete matching repository-gate JSON artifacts, non-recursive node-verifier commands, exact raw-report node/migration-unit identity, full repository-gate coverage, and a canonical manifest transition limited to top-level status plus per-node status/evidence/legacy-retirement after the attested source revision pass; operation/source/target/map contract mutation in that same manifest, source/registry/verifier drift, aggregate-gate self-certification, cross-node report reuse, repository-authored online success, missing node/run id, non-JSON artifact, command/result/node/gate/run mismatch, failed result, and incomplete coverage fail
...
- known gaps:
  ...
  - WebUI, Android-device, and legacy no-touch verifier outputs do not yet have a source-bound external provenance owner; `online_verified` and `legacy_retired` therefore fail closed
