...
| `node_pairing.project_to_ui` | bound | `cargo test -p freehand-node` covers delegated progress, node-owned slave-turn publication, status projection, permission rejection, and debug observation tests | `cargo test -p freehand-node` covers node boundary status/progress/subscription smokes on node-owned projection types | `cargo run -p xtask -- gates check` covers project boundary plus UI projection resource-map/mainline binding; live UI transport integration remains a documented gap outside this bound source edge |
