...
- WebUI JS/CSS asset smoke locks mobile v3 phone-first structure: visible mobile chrome must not render desktop internal strips (`topbar-strip`, `strip-session`, `strip-turn`, `strip-cwd`, `slave-drawer`, `slave-chip`), selected session/turn verifier truth must live on shell `data-*`, phone bubbles must be lightweight, tool blocks must be compact semantic rows, and focused composer may enlarge the textarea and reveal cancel but must not re-open the full attachment/CWD/model/status strip into the conversation flow
