...
- WebUI online black-box coverage must assert the session list text does not expose internal lifecycle session ids such as `master-lifecycle-*`; direct debug queryability belongs to `ui.protocol` coverage
