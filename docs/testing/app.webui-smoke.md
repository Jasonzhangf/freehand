...
- resource operations:
  ...
  - `ui_projection.bind_protocol_call`
...
| `ui_projection.bind_protocol_call` | bound | `node scripts/verify-webui-foundation-contracts.mjs` validates the generated ADP constructor manifest and rejects handwritten frame-class drift | `node --check apps/freehand-server/assets/webui/app-shell/adp-client.js && node --check apps/freehand-server/assets/webui/legacy-monolith.js` plus the WebUI server asset test verify generated constructors are consumed by the client | `node scripts/verify-webui-mobile-ui-tree-online.mjs` proves the served client completes live generated query, command, and subscription calls |
...
- lifecycle path under test:
  ...
  - ADP connection scope classifies loopback Host as local and non-loopback or Relay-marked access as remote before invoking the scoped runtime query port
  ...
  - the mobile Agent sheet has two mutually exclusive projections: without a selected session it shows one row per configured Agent and navigates through the browser-reachable local or Relay URL; with a selected session it shows only that session's Worker task rows
  ...
  - WebUI asset smoke locks Chinese lifecycle labels in the Header instead of unexplained `A/R/B/C` abbreviations and locks cache-hit rendering to raw read/total counters with exactly three decimal places
  ...
  - a terminal failed Worker turn remains historical truth, but when the same TaskBoard-projected Worker session has a later `Success` turn and the task is owner-terminal `closed`, normal SessionDetail presentation must relabel the older turn `历史失败 · 后续已恢复`, suppress its duplicate raw provider-error rows, and mark the cycle `data-recovery-state="historical_failure_recovered"`; the frozen same-key cycle must refresh when TaskBoard owner truth first proves recovery and when debug-details mode toggles, so debug mode can still reveal the original failure. Negative coverage must keep a latest failure red when there is no later success, when the later success precedes it, or while the task is not closed.
...
- Server APK update route smoke must prove `/android/update.json` serves explicit 运行时 env override or compiled sidecar truth with no-store cache headers, `/android/freehand-android.apk` serves the signed staged APK, and that a missing sidecar does not return a hardcoded old `versionCode` as a false current-version success.
  ...
  - WebUI ADP pre-handshake query is rejected before runtime query port; query-as-command failure does not mutate state and renders failure
  ...
  - WebUI mobile Home asset smoke requires the owner-backed Agent summary strip to remain visible on `home_dashboard`, because it is the only portrait entry into the configured Agent directory; the Home route may hide transcript/session-relation surfaces but must not hide this Agent entry
  - WebUI mobile Home Agent-directory edge smoke requires `home.open_agent_directory` to open the global configured-Agent sheet with no selected session; `session.open_agent_sheet` remains the separate selected-session Worker-task edge, and non-Home/non-SessionDetail routes must not open either sheet
...
- WebUI JS/CSS asset smoke locks no same-tool card normalization, no DOM text de-duplication, immediate composer clearing on submit, Up/Down input history recall, submit/dispatch 等待中 timers, typed model-response 等待中 timers from protocol projection, phase-key timer reset, absence of UI-inferred 等待中-model timers, current-live-turn-only animation gating, static rendering for selected-transcript `model_request` rows that are not current live, neutral non-animated inactive 等待中 state, 等待中 animation assets, compact terminal tool state dots, protocol-structured tool cards that render `display.action`, `display.kind`, `display.target`, `display.result_summary`, `display.summary`, all `display.fields`, `display.diff`, command fields, and raw detail lines without reading obsolete `display.result`, filtering generic tool results, or compressing repeated tool text; every tool card exposes only a visible `复制结果` action and final successful summary cards expose `复制` plus `加入记忆`
- WebUI JS asset smoke locks chronological tool/result presentation: a turn with tool activity and assistant/final content renders the tool card before the final summary, rather than sorting terminal summary content ahead of tool output
...
- Server asset smoke locks the Android ready-host bridge symbols `window.__freehandOpenAndroidComposerForReadyHost`, `openAndroidComposerForReadyHost`, `scheduleAndroidComposerFocus`, `focusAndroidComposerAfterReadyLayout`, and `composerCanReceiveAndroidInput`; `node --check apps/freehand-server/assets/webui/legacy-monolith.js` covers syntax
- Server asset smoke locks the New dialog `t-modal` open/close hook cleanup, status `aria-live`/`is-updating` feedback, visible focus-visible rings, and 44px mobile New/action drawer touch targets
...
- WebUI root/asset smoke asserts server-stamped versioned HTML/asset URLs (`WEBUI_ASSET_VERSION`, no residual `__WEBUI_ASSET_VERSION__`) plus `Cache-Control: no-store, max-age=0` on both root HTML and assets; Android online proof must force-stop/relaunch the app and show the new dashboard behavior without clearing app data
...
- Android true-device launch proof must force-stop/relaunch the installed app, assert the canonical WebUI route is `session_detail`, composer geometry is visible and focused, and `dumpsys input_method` reports a served input connection with `mShowInputRequested=true`; screenshot evidence must be saved under `artifacts/`.
...
- WebUI mobile accessibility regression coverage must assert the closed sidebar and inspector are `inert` at portrait widths, the selected drawer is not inert after opening, and desktop layouts leave both drawers non-inert
...
- WebUI selected-session lifecycle refresh treats `ToolPending` as owner-truth watchdog activity only while waiting tool/model activity or a lifecycle-wakeable task/timer owner projection exists; once TaskBoard shows no lifecycle-wakeable task and TimerList shows no open timer for the session, the same `ToolPending` turn renders `等待用户选择`; before timer projection arrives, absence of waiting tool/model/task evidence must not keep the stale lifecycle label, no longer counts as a running lifecycle/Agent, blocked-only child tasks also do not keep Home `正在运行` active, and remains exitable. Historical `ToolPending` cards stop driving refresh after terminal truth becomes selected; same-key frozen `.turn-cycle-card` reuse may refresh only this `waiting_lifecycle` -> `waiting_user` classification metadata/content, and the terminal-detail prefix is normalized from `Waiting for lifecycle:` to user-choice wording, so the visible card cannot keep the obsolete lifecycle label after owner projections load.
...
- WebUI online Worker-history recovery proof runs `node scripts/verify-worker-recovered-history-online.mjs`; it opens the TaskBoard-projected Worker session for a task whose first execution failed and later execution closed successfully, asserts the old cycle is marked `historical_failure_recovered`, asserts normal visible text contains `历史失败 · 后续已恢复` but not the raw 401 payload, and asserts the current Worker/task status remains `已关闭`; toggling debug details must replace the frozen same-key recovery projection and make the original failure inspectable without changing task/session truth.
...
- Online gate: `node scripts/verify-webui-mobile-ui-tree-online.mjs` now fetches the expanded module asset list and must prove Home/SessionDetail mutual exclusion, one-row/two-panel Home dashboard, fixed history buckets, Settings drilldown, Header Worker rail status/duration rows plus click-to-expand details from fixed TaskBoard truth, and no portrait overflow.
  ...
  - freehand-server ADP tests lock first-frame handshake (`adp_handshake_required`, duplicate-handshake rejection, post-handshake query success), QueryMasterPoll reaching runtime query port while RunMasterPoll/ApplyExecutionFact are rejected before owner query with `direct_task_mutation_forbidden`, public command ingress rejecting internal ApplyExecutionFact with `adp_command_not_public`, plus positive admission only when a server-owned `FREEHAND_ADP_INTERNAL_COMMAND_TOKEN` matches the client capability; unmatched client-advertised capabilities are not accepted as internal authorization.

- Focused ADP transport proof: `CARGO_TARGET_DIR=/tmp/freehand-target-adp-version cargo test -p freehand-server adp -- --test-threads=1` covers first-frame handshake gating, duplicate handshake rejection, post-handshake query success, and query mutation rejection; Relay transport is covered by the independent `relay.transport` test design; `node --check apps/freehand-server/assets/webui/app-shell/adp-client.js` plus `node --check apps/freehand-server/assets/webui/generated/adp-protocol.js` covers split client and generated constructor syntax.
...
| Resource Operation | Status | White-Box | Module Black-Box | Project Black-Box |
| --- | --- | --- | --- | --- |
| `app.webui-smoke.render.tool_card_actions` | bound | renderToolSection emits only `.tool-chat-copy`; successful final summary rows emit `.final-summary-copy` and `.final-summary-memory`, with memory sent through `AddToMemory` using session_id / turn_id / content | `node --check apps/freehand-server/assets/webui/legacy-monolith.js` | `node scripts/verify-webui-live-tool-render-online.mjs` proves tool cards expose copy-only and final summary cards expose copy plus memory |
