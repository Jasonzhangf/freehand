...
- WebUI online black-box coverage must capture screenshots and DOM state for the required viewport matrix and assert `body[data-layout-shape]`, shell `data-layout-shape`, composer visibility, message list visibility, and no historical live animation after terminal refresh
