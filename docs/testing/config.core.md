...
- lifecycle path under test:
  ...
  - compile a validated account-scoped non-secret document into an in-memory effective LoadedConfig without rewriting local canonical config
...
| --- | --- | --- | --- | --- |
| `config.compile_agent_relay_connection` | bound | `cargo test -p freehand-config agent_relay -- --nocapture --test-threads=1` covers atomic field admission, URL validation, token-env resolution, and missing/empty token rejection | `cargo test -p freehand-config -- --nocapture` exercises selected-agent projection with configured and absent Relay connection truth | `scripts/verify-remote-relay-local-online.sh` starts an Agent from an explicit Relay URL/token and proves the outbound tunnel online |
...
| `config.project_shared_account_config` | bound | `cargo test -p freehand-config export_shared_account_config_ -- --nocapture` proves env-auth-only provider export, compatible model-group filtering, endpoint projection, and secret-shaped value rejection | `cargo test -p freehand-runtime --lib runtime_account_config_sync_push_conflict -- --nocapture` consumes the exporter through the real push dispatch | `scripts/verify-relay-account-config-smoke.sh` must prove the deployed client uses this exporter and no secret value crosses the Relay boundary |
| `account_config_document.apply_shared_account_config` | bound | `cargo test -p freehand-config apply_shared_account_config_ -- --nocapture` proves env-auth provider override, compatible model-group compilation, owner-prefixed shared relay/daemon merge, and invalid route rejection | `cargo test -p freehand-runtime --lib runtime_account_config_sync_pull_applies_shared_provider_to_effective_config runtime_account_config_sync_bootstrap_applies_existing_synced_mirror -- --nocapture --test-threads=1` consumes the apply path through real pull dispatch and existing-mirror startup | `scripts/verify-relay-account-config-smoke.sh` plus the Claw online client smoke must prove a synced document applies to effective runtime config without rewriting `config.toml` |
...
- white-box plan:
  ...
  - assert a validated account-shared document can override a same-id local provider only when auth is env-only, compile compatible shared model groups, merge shared relay/daemon entries under `shared:` ids, and leave canonical local config unchanged
  - assert a shared model group referencing an unknown provider and a shared provider carrying non-env auth fail before any effective config is returned
...
- sync status between design and implementation:
  ...
  - `cargo test -p freehand-config apply_shared_account_config_ -- --nocapture` covers account-shared-to-effective compilation and rejection
