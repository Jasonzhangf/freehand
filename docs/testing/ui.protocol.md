...
| `search_evidence.project_to_ui` | pending | `cargo test -p freehand-ui-protocol search_evidence -- --nocapture` covers `UiSearchEvidenceProjection` typed projection of hosted discovery, camo verification, access attempts, claims, and unconfirmed evidence without parsing provider or camo raw text | `cargo test -p freehand-runtime live_bridge -- --nocapture` covers runtime projection of search evidence into UI DTOs through `merge_hosted_search_activities` and `UiProtocolState::apply_search_evidence` | `node scripts/verify-provider-hosted-web-search-online.mjs` proves browser DOM renders only `UiSearchEvidenceProjection` fields and never parses provider/camo raw text |
...
- sync status between design and implementation:
  ...
  - per-turn usage projection is regression-locked by `usage_projection_carries_cache_hit_and_context_tokens`, `legacy_anthropic_usage_projection_reconstructs_total_input`, and `legacy_usage_projection_preserves_cache_subset_denominator` (input/output/reasoning/cache tokens, cache-hit-rate bps, normalized context tokens, compacted-token counter, and both persisted legacy usage shapes without `normalized_input_tokens`)
