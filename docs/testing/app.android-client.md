...
5. `MainActivity::onCreate` configures WebView and loads `HostConfig.baseUrl + "/?client=android-webview"` immediately.
...
9. `MainActivity::reportCanonicalWebUiLayout` logs canonical WebUI DOM evidence.
...
- `HostConfigTest` covers endpoint URL construction used by WebUI URL loading.
...
- Acceptance evidence must include a screenshot and logcat showing daemon WebUI selectors/layout (`data-webui-shell=true`, `layoutClient=android-webview`), not native Android chrome.
