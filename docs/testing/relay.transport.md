...
| `relay_control_tunnel.admit_data` | bound | `cargo test -p freehand-relay -- --nocapture` proves current-generation admission and stale-generation rejection | `cargo test -p freehand-relay --test relay_http_blackbox -- --nocapture` proves the version-matched client establishes data only after control acknowledgement | `scripts/verify-relay-deployment-smoke.sh` proves the real Agent client data channel |
| `relay_control_tunnel.admit_error` | bound | `cargo test -p freehand-relay tunnel::tests::error_attachment_requires_control_in_the_same_registry_mutation -- --nocapture` proves missing control rejects before error insertion and `cargo test -p freehand-relay tunnel::tests::stale_control_generation_cannot_attach_data_or_error_channels -- --nocapture` proves stale rejection | `cargo test -p freehand-relay --test relay_http_blackbox -- --nocapture` proves the version-matched client establishes its error return path after control acknowledgement; stale-header admission is covered at the registry boundary | `scripts/verify-relay-deployment-smoke.sh` proves the real Agent client keeps a control-owned error channel through proxied traffic |
...
| `relay_data_tunnel.accept_generation` | bound | `cargo test -p freehand-relay stale_data_generation -- --nocapture` proves only the current data attachment generation can deliver into the live exchange registry | `cargo test -p freehand-relay --test relay_http_blackbox -- --nocapture` exercises the version-matched live data socket path; stale-generation response rejection is locked at the registry boundary by the white-box command | `scripts/verify-relay-deployment-smoke.sh` verifies the deployed current-channel usability after exact service replacement |
| `relay_error_tunnel.correlate` | bound | `cargo test -p freehand-relay tunnel::tests::failure_removes_only_the_active_exchange_and_unknown_failure_is_explicit -- --nocapture` proves exact exchange removal and explicit unknown failure; `cargo test -p freehand-relay tunnel::tests::control_replacement_fails_old_exchange_and_stale_cleanup_is_fenced -- --nocapture` proves replacement fails old in-flight work and stale cleanup cannot detach the current generation | `cargo test -p freehand-relay --test relay_http_blackbox -- --nocapture` proves Agent bridge failures reach the typed error attachment without becoming success | `scripts/verify-relay-deployment-smoke.sh` keeps the control, data, and error attachments live through a real proxied request |
...
| positive | a newly authenticated control generation replaces stale control truth, removes old data/error attachments, and fails old pending exchanges through the typed error chain | a stale control/data/error cleanup cannot detach the current generation or its attachments |
...
| positive | the current server-issued control generation admits both data and error attachments | dependent typed channels remain available for the live control owner |
| negative | a replaced control generation cannot attach either a data or error channel even while a newer control generation is active | a stale control socket cannot race the replacement socket and seize its dependent channel slots |
...
- Positive module black-box: authenticated Agent control and data channels register one live identity; streamed HTTP request/response chunks preserve bytes and status; ADP frames remain opaque.
- Negative module black-box: wrong account token, mismatched Agent identity, cross-identity response/error injection, data-before-control, expired/disconnected tunnel, malformed node order, unknown exchange, incomplete HTTP/WebSocket response, and Agent error all enter `RelayError*` and never project success. Stale control/data/error generation rejection remains white-box registry coverage until a dedicated WebSocket replacement fixture is added.
