...
| `task.cancel` | bound | `cargo test -p freehand-task cancel_assigned_task_releases_agent_and_rejects_resume -- --nocapture` verifies non-terminal cancellation releases assignment and prevents resume | `cargo test -p freehand-runtime runtime_dispatches_session_rollback_into_effective_ui_projection -- --nocapture --test-threads=1` verifies session rollback cancels child tasks created by the rolled-back parent turn and does not cancel retained-turn children | `scripts/verify-webui-path-diagnostic-online.mjs` verifies fixed-session rollback does not let historical child tasks consume the next WebUI run |
