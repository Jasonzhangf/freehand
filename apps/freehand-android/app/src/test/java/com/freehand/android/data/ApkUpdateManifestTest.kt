...
class ApkUpdateManifestTest {
    ...
    @Test
    fun `relay manifest root relative apk url keeps deployment prefix`() {
        val manifest = ApkUpdateManifest.parse(
            """
            {
              "versionCode": 3,
        ...
        assertEquals(
            "http://relay.example/company/relay/updates/freehand-android.apk",
            plan?.apkUrl,
        )
    }

    @Test
    fun `relay manifest root relative apk url preserves nested path under prefix`() {
        val manifest = ApkUpdateManifest.parse(
            """
            {
              "versionCode": 3,
              "apkUrl": "/relay/updates/releases/freehand-android.apk",
              "sha256": "979906f579118625c1f57e6db1eef8f055475ed884abb93934e180d0b8f14611",
              "size": 45000000,
              "signerSha256": "ecd63a2c2070970735cc079b0bb090427ca0b59200da0ebc07c80b50a1dfffda"
            }
        ...
        assertEquals(
            "http://relay.example/company/relay/updates/releases/freehand-android.apk",
            plan?.apkUrl,
        )
    }

    @Test
    fun `relay manifest non relay apk path is not rebased under prefix`() {
        val manifest = ApkUpdateManifest.parse(
            """
            {
              "versionCode": 3,
        ...
        assertEquals(
            "http://relay.example/android/freehand-android.apk",
            plan?.apkUrl,
        )
    }
